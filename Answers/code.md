...
```
*Question: Given a string, write a function that can reorganize it into a Palindrome. If it is not a palindrome, return -1*
```javascript
function canBePalindrome(str) {
    // set the string to lower case first
    var str = str.toLowerCase();
...
```
*Question Given an array of strings now, this function should reorganize all of them into Palindromes. If it is not a palindrome, return -1 for that value*
```javascript
...
canBePalindromeArray(['abcac', 'abc', 'gshgh', 'dd ff']); //["acbca", -1, "ghshg", "df fd"]
```
